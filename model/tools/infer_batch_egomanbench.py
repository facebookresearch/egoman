...
video_id = "egomanbench"
img_dir = f"../data/{video_id}_imgs"
...
def main(annotations_all):
    ...
    for batch_indices in chunk_list(indices, bs):
        ...
        for ori_cur_anno in batch_ann:
            ...
            past_motion = np.zeros((5, 2, 7))
            if cur_anno["dataset"] not in ["egoexo", "nymeria"]:
                past_motion_query = "<past_motion>" * 5
                cur_anno["question"] = ori_cur_anno["question"] = (
                    f"Given the past wrist motion: {past_motion_query}. Where will the hands move to manipulate {ori_cur_anno['object_name']} with {ori_cur_anno['manipulating_hand']} hand?<HOI_QUERY>"
                )
                cur_anno["intention"] = (
                    f"manipulate {ori_cur_anno['object_name']} with {ori_cur_anno['manipulating_hand']} hand"
                )
                all_quat = np.hstack(
                    [
                        align_quat_to_styleA(
                            np.array(cur_anno["left_hand_traj"])[..., 3:], "left"
                        ),
                        align_quat_to_styleA(
                            np.array(cur_anno["right_hand_traj"])[..., 3:], "right"
                        ),
                    ]
                )[:25]
                all_traj = np.hstack(
                    [
                        np.array(cur_anno["left_hand_traj"])[..., :3],
                        np.array(cur_anno["right_hand_traj"])[..., :3],
                    ]
                )[:25]
                if len(all_traj) < 5:
                    continue
                past_motion[..., :3] = np.round(
                    np.array(all_traj)[:5].reshape(-1, 2, 3), 3
                )
                past_motion[..., 3:] = np.round(
                    np.array(all_quat)[:5].reshape(-1, 2, 4), 4
                )
            else:
                if "question" in ori_cur_anno:
                    cur_anno["question"] = ori_cur_anno["question"]
                    if ("left hand" in cur_anno["value"][2].lower()) and (
                        "right hand" not in cur_anno["value"][2].lower()
                    ):
                        cur_anno["question"] = cur_anno["question"].replace(
                            cur_anno["intention"],
                            cur_anno["intention"] + " with left hand",
                        )
                    elif ("right hand" in cur_anno["value"][2].lower()) and (
                        "left hand" not in cur_anno["value"][2].lower()
                    ):
                        cur_anno["question"] = cur_anno["question"].replace(
                            cur_anno["intention"],
                            cur_anno["intention"] + " with right hand",
                        )
                    else:
                        cur_anno["question"] = cur_anno["question"].replace(
                            cur_anno["intention"],
                            cur_anno["intention"] + " with both hands",
                        )
                past_motion[..., :3] = np.array(cur_anno["value"][-2][:5]).reshape(
                    -1, 2, 3
                )
                past_motion[..., 3:] = np.array(cur_anno["value"][-1])[:5].reshape(
                    -1, 2, 4
                )
            ...
            for bx, (pred_txt, gen_ids, hids, dd) in enumerate(
                ...
                if (
                    ...
                    if len(local_results) >= save_every:
                        tmp_path = CONFIG["log_path"].replace(
                            ".pkl", f".rank{rank}.tmp.pkl"
                        )
...
if __name__ == "__main__":
    annotations_all = pickle.load(
        open("../data/egoman_dataset/egoman-test-final.pkl", "rb")
    ) + pickle.load(open("../data/egoman_dataset/hot3d-grab-anno-clean.pkl", "rb"))
    main(annotations_all)

# BATCH_SIZE=1 SAVE_EVERY=50 MODEL_NAME=EgoMAN-7B torchrun --nproc_per_node=1 tools/infer_batch_egomanbench.py
