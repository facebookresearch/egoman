...
def discretize(value, min_bound, max_bound, num_bins=256):
    bin_size = (max_bound - min_bound) / num_bins
    return int((value - min_bound) / bin_size)
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer, data_args):
        ...
        self.vis_emb_dict = pickle.load(
            open(
                f"../data/egoman_dataset/egoman_dinov3_features.pkl",
                "rb",
            )
        )
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for attempt_idx in range(num_base_retries):
            try:
                ...
                # sleep 0.5s in case it is a cloud disk issue
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        sources[0]["wrist_pose"] = np.zeros((58, 23)) - 1000
        sources[0]["pre_wrist_pose"][..., :6] = np.array(sources[0]["value"][-2][:5])

        sources[0]["wrist_pose"][:3, :11] = np.array(sources[0]["value"][0])
        sources[0]["wrist_pose"][
            3 : 3 + len(np.array(sources[0]["value"][-2])), 1:7
        ] = np.array(sources[0]["value"][-2])

        quat_ori = torch.from_numpy(np.array(sources[0]["value"][-1]))
        quat_ori = quat_ori.reshape(-1, 4)
        ...
        sources[0]["pre_wrist_pose"][..., 6:] = quat_ori[:5]
        sources[0]["wrist_pose"][
            3 : 3 + len(np.array(sources[0]["value"][-1])), 11:
        ] = quat_ori
        start_end_timestamp = (
            torch.from_numpy(sources[0]["value"][0][..., 0] * 4.5 * 10).round().long()
        )
        time_idx = start_end_timestamp[1]
        start_contact_loc_quat = torch.stack(
            [quat_ori[5], quat_ori[5 + time_idx], quat_ori[-1]], dim=0
        )
        sources[0]["wrist_pose"][:3, 11:] = start_contact_loc_quat.numpy()
        assert len(np.array(sources[0]["value"][-2])) == len(
            np.array(sources[0]["value"][-1])
        )
        phrase_str = sources[0]["value"][2]
        key = sources[0]["image"] + "_" + phrase_str
        vis_key = sources[0]["image"]
        future_act = torch.Tensor(self.act_emb_dict[key]["emb"]).reshape(-1)
        dino_vis_emb = torch.from_numpy(self.vis_emb_dict[vis_key]).reshape(-1)
        ...
        if (
            ...
            sources[0]["wrist_pose"][1:3, 4:7] = -1000
            sources[0]["wrist_pose"][1:3, 9:11] = -1000
            sources[0]["wrist_pose"][1:3, 17:] = -1000
            ...
            sources[0]["wrist_pose"][1:3, 1:4] = -1000
            sources[0]["wrist_pose"][1:3, 7:9] = -1000
            sources[0]["wrist_pose"][1:3, 11:17] = -1000
        ...
        data_dict = preprocess_qwen_2_visual(
            ...
            ori_sources=sources,
            max_scale=self.data_args.max_scale,
            z_neg=self.data_args.z_neg,
        )
